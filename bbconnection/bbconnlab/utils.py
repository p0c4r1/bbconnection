...
class MiddlewareTask(object):
    ...
    def insert_order_result(self,order_id,test_id):
        ...
        e_ot_child0 = models.OrderResults.objects.filter(order_id=order_id,test_id=test_id)
        if not e_ot_child0.count():
            unit = self.get_test_unit(test_id)
            ref_range = self.get_ref_range(test_id)
            i_test = models.OrderResults(order_id=order_id,test_id=test_id,unit=unit,ref_range=ref_range,sample_id=sample[0].id)
            i_test.save()
    ...
    def get_ref_range(self,test_id):
        ...
        self.order = order
        if self.is_active():
            refrange = models.TestRefRanges.objects.filter(test_id=test_id).values('any_age','gender','age_from_type','age_from','age_to','operator','operator_value','lower','upper','alfa_value','special_info','gender_id')
            if refrange.count()>0:
